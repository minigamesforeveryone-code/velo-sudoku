...
> - Section 2: Corrected the description of data storage — beyond the device's memory, data may be copied to the User's own Google account (Android Auto Backup, and cloud save once the game is connected to Play Games). The Developer still has no backend server and no access to these copies.
...
> - Section 5: Added management of cloud copies (deleting the Play Games copy, Android Auto Backup).
...
* **Copies on the User's Google account:** Two mechanisms may additionally copy this data **to the User's own Google account** — never to the Developer, who has no backend server and no access to these copies: **Android Auto Backup** (an Android system feature that copies app data to the User's Google Drive) and, once the game is connected to **Google Play Games**, **cloud save** (see section 3D).
...
* **Android Auto Backup:** managed in the device's system settings (*Android Settings -> Google -> Backup*) and in the User's Google Drive.
